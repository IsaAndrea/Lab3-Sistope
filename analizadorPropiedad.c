...
/* Procedimiento que verifica y muestra por pantalla si la imagen corresponde o no a nearly black. Además, señala a cuánto 
    corresponde el porcentaje de pixeles negros. 
   ENTRADA:
...
/* Función principal de analizadorPropiedad. Define el pid del hijo a crear y una tubería, para luego realizar la lectura de la 
    tubería de binarizadorImagen, obteniendo los datos correspondientes de las cabeceras binformacion y bcabecera, así como de 
    binarios_color. De esta forma, es posible asignar los parámetros necesarios a verificarNearlyBlack.
    Una vez creado el hijo, éste procede a ejecutar creadorImagen. Por otro lado, el padre escribe en la tubería los datos 
